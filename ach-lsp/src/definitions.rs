...
pub fn prepare_rename(source: &str, byte_offset: usize) -> Option<(Range, String)> {
    ...
    Some((
        Range {
            start: Position::new(line_start, col_start),
            end: Position::new(line_end, col_end),
        },
        name,
    ))
}
...
/// Returns a list of (Range, new_name) pairs for building a WorkspaceEdit.
pub fn rename(source: &str, byte_offset: usize, new_name: &str) -> Vec<(Range, String)> {
    find_references(source, byte_offset)
        .into_iter()
        .map(|range| (range, new_name.to_string()))
        .collect()
}
...
mod tests {
    ...
    fn rename_fn_all_occurrences() {
        ...
        assert!(edits.iter().all(|(_, name)| name == "triple"));
    }
